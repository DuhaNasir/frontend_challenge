...
const PreviousPolls = () => {
  ...
  useEffect(() => {
    const fetchAllPolls = async () => {
      try {
        ...
        setPolls(Array.isArray(data) ? data.reverse() : []);
      } catch (error) {
        console.error("حدث خطأ أثناء جلب التصويتات:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <h1 className="text-3xl font-black text-gray-800 mb-8 flex items-center gap-2">التصويتات العامة 🗂️</h1>
      ...
      {polls.length === 0 ? (
        <div className="bg-white p-12 rounded-3xl shadow-sm border-2 border-dashed border-gray-200 text-center flex flex-col items-center">
          ...
          <h2 className="text-xl font-bold text-gray-700 mb-2">لا توجد تصويتات حالياً</h2>
          <Link to="/" className="bg-primary-pink hover:bg-primary-pink-hover text-white px-6 py-3 rounded-xl font-bold transition-colors mt-4">
            كن أول من ينشئ تصويتاً!
          </Link>
        </div>
      ) : (
        <div className="space-y-6">
          {polls.map((poll) => {
            const currentId = poll.pollId || poll.id;
            const isSending = sendingId === currentId;
            const isSuccess = successId === currentId;

            return (
              <div key={currentId} className="bg-white p-6 rounded-3xl shadow-sm border-2 border-gray-100 hover:border-primary-yellow transition-all relative overflow-hidden">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pr-4">
                  <div className="flex-1">
                    ...
                    <div className="flex gap-4 text-sm font-bold text-gray-500">
                      ...
                      <span className="text-gray-400">الإيميل: {poll.email}</span>
                    </div>
                  </div>
                  <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
                    <Link to={`/results/${currentId}`} className="flex items-center gap-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2.5 rounded-xl text-sm font-bold transition-colors flex-1 justify-center">
                      <ExternalLink size={16} /> النتائج
                    </Link>
                    <button onClick={() => sendEmail(currentId)} disabled={isSending || isSuccess} className={`flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-sm font-bold transition-colors flex-1 justify-center ${isSuccess ? 'bg-green-100 text-green-700' : 'bg-primary-pink hover:bg-primary-pink-hover text-white'}`}>
